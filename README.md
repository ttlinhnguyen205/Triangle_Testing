...
```powershell
cd D:\TriangleTesting
```

Chạy kiểm thử bằng Maven:

```powershell
mvn test
```

Hoặc chạy lại toàn bộ test và sinh báo cáo coverage:
...
Có thể mở nhanh bằng lệnh:

```powershell
start D:\TriangleTesting\target\site\jacoco\index.html
```

Kết quả coverage đạt:

| Loại coverage                    | Kết quả |
| -------------------------------- | ------- |
| Instruction / Statement Coverage | 100%    |
| Branch Coverage                  | 100%    |
| Line Coverage                    | 100%    |
| Method Coverage                  | 100%    |
| Class Coverage                   | 100%    |
